...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
            ...
            if (category) {
                ...
                try {
                    ...
                    if (error.code === 'ENOENT') {
                        throw new McpError(ErrorCode.InvalidParams, `カテゴリー "${category}" が見つかりません`);
                    }
                    throw error;
                }
            }
        ...
        if (request.params.name === 'search_document') {
            ...
            if (!filename) {
                throw new McpError(ErrorCode.InvalidParams, 'ファイル名を指定してください');
            }
            let filePath = null;
            ...
            try {
                ...
                if (error.code === 'ENOENT') {
                    throw new McpError(ErrorCode.InvalidParams, `ファイル "${filename}" が見つかりません`);
                }
                throw error;
            }
        }
        if (request.params.name === 'fetch_general') {
            ...
            try {
                ...
                if (error.code === 'ENOENT') {
                    throw new McpError(ErrorCode.InvalidParams, 'general.md が見つかりません');
                }
                throw error;
            }
        }
        ...
        throw new McpError(ErrorCode.InternalError, `ツールの実行中にエラーが発生しました: ${error}`);
    }
});
async function main() {
